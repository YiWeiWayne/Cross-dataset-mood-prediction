...
if action == 'melSpec_lw':
    ...
    source_execute_name = save_path + '/(' + action + ')' + source_dataset_name + 'DCGAN_20180629.0304.48'
elif action == 'rCTA':
    source_execute_name = save_path + '/(' + action + ')' + source_dataset_name + 'DCGAN_20180629.0307.47'
sec_length = 29
...
if discriminator_net == 'nn':
    ...
    discriminator_activations = ['elu', 'elu', 'elu', 'sigmoid']
elif discriminator_net == 'cnn_bn':
    discriminator_units = [32, 64, 128, 1]
    discriminator_activations = ['elu', 'elu', 'elu', 'sigmoid']
...
para_line.append('use_shared_s_t:' + str(use_shared_s_t) + '\n')
...
if 'cnn' in discriminator_net:
    ...
    para_line.append('discriminator_bn :' + str(regressor_bn) + '\n')
...
for emotion in emotions:
    ...
    if discriminator_net == 'nn':
        ...
        discriminator_model = Model(inputs=source_or_target_tensor,
                                    outputs=model_structure.cnn_classifier(x=source_or_target_tensor, input_channel=1,
                                                                           units=discriminator_units,
                                                                           activations=discriminator_activations,
                                                                           kernels=discriminator_kernels,
                                                                           strides=discriminator_strides,
    ...
    if use_shared_s_t:
        target_feature_tensor = source_extractor.input
        target_feature_extractor = source_extractor.output
                ...
                for f in files:
                    if os.path.splitext(f)[1] == '.h5' and \
                        ...
                        if load_weights_source_feature_extractor:
                            print('set source')
                            source_extractor.set_weights(model.get_weights()[:-(2+4)*len(regressor_units)+4])
                        # target_discriminator_model's weights will be set in the same time.
                        if load_weights_target_feature_extractor:
                            print('set target')
                            target_extractor.set_weights(model.get_weights()[:-(2+4)*len(regressor_units)+4])
                        if load_weights_source_classifier:
                            print('set classifier')
                            regressor_model.set_weights(model.get_weights()[-(2+4)*len(regressor_units)+4:])
    ...
    as_feat = target_extractor.predict(Source_Train_X, batch_size=4)
    ...
    np.save(model_path + str(0).zfill(4) + '@as_feat.npy', as_feat)
    print(s_feat.shape)
    print(t_feat.shape)
    print(at_feat.shape)
    # 10
    ...
    for epoch in range(0, epochs):
        ...
        np.save(model_path + str(epoch+1).zfill(4) + '@as_feat.npy', as_feat)
