...
action_description = 'Change features to melSpec_lw, rCTA, pitch+lw and ' \
                     'rmsprop and ' \
                     'bigger batch size and enforced classifier'
...
para_line.append('classifier_layer_num:' + str(classifier_layer_num) + '\n')
for feature in features:
    ...
    for feature_index in range(0, len(features)):
        ...
        discriminator_model = Model(inputs=source_or_target_tensor,
                                    outputs=model_structure.domain_classifier(source_or_target_tensor,
                                                                              discriminator_units))
        discriminator_model.compile(loss=wasserstein_loss, optimizer=rms, metrics=['accuracy'])
        ...
        target_tensor = Input(shape=(encoded_size*3, ))
        ...
        source_extractor[0] = Model(inputs=source_feature_tensor[0], outputs=source_extractor_tensor[0])
        source_extractor[1] = Model(inputs=source_feature_tensor[1], outputs=source_extractor_tensor[1])
        source_extractor[2] = Model(inputs=source_feature_tensor[2], outputs=source_extractor_tensor[2])
        ...
        target_discriminator_model = Model(inputs=target_feature_tensor[feature_index],
                                           outputs=discriminator_model(target_extractor_tensor[feature_index]))
        target_discriminator_model.compile(loss=wasserstein_loss, optimizer=rms, metrics=['accuracy'])
        ...
        model_path = execute_name + '/' + emotion + '/' + features[feature_index] + '/'
        ...
        print('Training: ' + emotion + '/' + features[feature_index])
        for epoch in range(0, epochs):
            ...
            for t in range(0, total_training_steps):
                ...
                for i in range(k_d):
                    ...
                    sample_source_x[0], sample_source_x[1], sample_source_x[2], sample_source_y = next(source_data_generator)
                    sample_target_x[0], sample_target_x[1], sample_target_x[2], sample_target_y = next(target_data_generator)
                    ...
                    source_tensor_output = source_extractor[feature_index].predict(sample_source_x[feature_index])
                    target_tensor_output = target_extractor[feature_index].predict(sample_target_x[feature_index])
                ...
                for i in range(k_g):
                    ...
                    sample_target_x[0], sample_target_x[1], sample_target_x[2], sample_target_y = next(
                        target_data_generator)
                    sample_target_x0[0], sample_target_x0[1], sample_target_x0[2], sample_target_y0 = next(
                        target_data_generator)
                    target_y = -np.ones((len(sample_target_y), 1))
                    target_y0 = -np.ones((len(sample_target_y0), 1))
                    make_trainable(discriminator_model, False)
                    make_trainable(target_extractor_con, False)
                    make_trainable(target_extractor[feature_index], True)
                    loss_t0 = target_discriminator_model.train_on_batch(sample_target_x[feature_index], target_y)
                    loss_t1 = target_discriminator_model.train_on_batch(sample_target_x0[feature_index], target_y0)
                    loss_fake = np.add((np.add(loss_t0, loss_t1) / 2), loss_fake)
