...
para_line.append('enable_target_label:' + str(enable_target_label) + '\n')
if enable_target_label:
    para_line.append('label_index:' + str(label_index) + '\n')
...
for emotion in emotions:
    ...
    if enable_target_label:
        ...
        log_data = ADDA_funcs.log_dump_reg(model_path=model_path, run_num=0,
                                           target_classifier_model=target_classifier_model,
                                           val_x=Target_Train_X, val_y=Target_Train_Y,
                                           log_data=log_data, loss_reg=loss_reg, loss_fake=loss_fake,
                                           loss_dis=loss_dis,
    ...
    for epoch in range(0, epochs):
        ...
        for t in range(0, total_training_steps):
            ...
            if enable_target_label:
                progbar.add(batch_size * (k_g * 2 + k_d), values=[("loss_reg", loss_reg),
                                                                  ("loss_dis", loss_dis),
                                                                  ("loss_fake", loss_fake)])
            else:
